...
# Use for loop
#iterate over different degrees of freedom (do each degree 1,000 times)
#iterate over number of sim's
for(i in 3:(length(df_vector)+2)) {
  for(j in 1:n_sim) {
    
    train_data <- generate_sample(f, sample_size = 200, sigma = sigma) %>%
      dplyr::select(x, y)
    model_spline <- smooth.spline(x=train_data$x, y=train_data$y, df=i)
    predictions_augmented <- predict(model_spline, x=x0) %>%
      tbl_df() %>%
      rename(y_hat = y)
    predictionss[j] <- predictions_augmented$y_hat
    } 
  truth <- data_frame(
    x = rep(x0, n_sim),
    f_x = f(x),
