...
#outer for loop to run the df 

for(j in 3:99)
  {
  #create a prediction vector to store stuff
  ...
  for(i in 1:n_sim){
    ...
    #test data is a single value
    test_data <- data_frame(x=x0) #x0= 0.5
    
    #fit the model 
    model_spline <- smooth.spline(x=train_data$x, y=train_data$y, df=j)
 ...
 #     .[[".fitted"]]
    ...
    model_augmented <- predict(model_spline, x=test_data$x) %>%
      tbl_df() %>%
      rename(y_hat = y) 
    
    #prediction vector from previous lab
    predictions[i] = model_augmented$y_hat
  ...
  #To compute MSE, bias^2 and variance
  #make sure it's within the outer for loop 
  #- I had it outside the first time that's why it didn't work
  
  MSE[j-2] = mean((truth$y - truth$y_hat)^2)
  bias_squared[j-2] = mean(truth$f_x - truth$y_hat)^2
  variance[j-2] = var(truth$y_hat)
}
