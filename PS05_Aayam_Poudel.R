...
for (deg_f in df_vector){
  ...
  for(i in 1:n_sim){
    ...
    #create a model with the current df value
    model_spline <- smooth.spline(train_data$x, train_data$y, df=deg_f)
    
    #fit the model and store the result in the predictions vector
  ...
  #put current bias in the right index of the vector
  bias_squared[deg_f-2] <- truth %>%
    summarise(bias_squared = (mean(f_x - y_hat))^2) %>%
    .[["bias_squared"]]
  
  #put current variance in the right index of the vector
