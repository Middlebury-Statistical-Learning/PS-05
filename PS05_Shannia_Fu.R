...
for(j in 3:99) {
  # keeps track of which df iteration we're on
  print(j)
  
  # creates truth data frame (to compare predictions to)
  truth <- data_frame(
    x = rep(x0, n_sim),
    f_x = f(x),
  ...
  # n_sim (10000) simulations of creating samples,
  # fitting models to the samples,
  # and storing predictions
  for(i in 1:n_sim) {
    # creates sample
    sample_train <- generate_sample(f, sample_size = sample_size, sigma = sigma)
    # fits spline, with df = j from outer loop
    model <- smooth.spline(sample_train$x, sample_train$y, df = j)
    # stores the prediction using the model at x = x0 = 0.5
    predictions[i] <- predict(model, x = x0)[2]
  }
  
  # put predictions next to true values
  truth <- truth %>%
    mutate(y_hat=as.double(predictions))
  # calculates mse
...
# prevents error of data fram being different length
df_vector <- c(1:99)
...
bias_sq_vs_var_plot <- 
  ...
  scale_x_log10()
bias_sq_vs_var_plot
ggsave(file="PS05_Shannia_Fu_bias_sq_vs_var.pdf", plot=bias_sq_vs_var_plot, width = 6, height=4)

# Plot MSE, bias_squared, variance
MSE_plot <- 
  ggplot(results_tidy, aes(x=df, y=value, col=type)) + 
  # added xlim because I changed df from 1 to 99, so it'll give you an error, sorry
  # error - (Removed 6 rows containing missing values (geom_path).)
  xlim(3, 99) +
  geom_line()
MSE_plot
ggsave(file="PS05_Shannia_Fu_MSE.pdf", plot=MSE_plot, width = 6, height=4)
